...
def markdown_to_html(file):
    ...
    write_to_html(mdt, html_f, htmlf_name)                                  #     writing the converted table to .html file    #
            ...
            if re.search('^\*[^\*]', mdt[row][word]): 
                if re.search('[^\*]\*$', mdt[row][word]): 
                    ...
                    if re.search('[^\*]\*$', mdt[row][word]): 
                        ...
                        while not re.search('[^\*]\*$', mdt[row][word]) and row < len(mdt)-1: 
                            row += 1

                            while not re.search('[^\*]\*$', mdt[row][word]) and word < len(mdt[row])-2: 
                                word+=1
            ...
            if re.search('^\*{2}[^\*]', mdt[row][word]): 
                if re.search('[^\*]\*{2}$', mdt[row][word]): 
                    ...
                    if re.search('[^\*]\*{2}$', mdt[row][word]): 
                        ...
                        while not re.search('[^\*]\*{2}$', mdt[row][word]) and row < len(mdt)-1: 
                            row += 1
